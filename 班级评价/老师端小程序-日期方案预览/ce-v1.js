...
ceTodayValidate=function(e){
  ...
  if(n<e.unitTenths){toast('单次'+e.type+'不能低于默认值'+(e.unitTenths/10).toFixed(1)+'分');return false}
  if(n>1000){toast(ceTodayMaxMessage);return false}
...
ceEnterToday=function(names,student){
  ...
  ceTodaySearchQuery='';ceState.category=ceCategories[0];ceState.subcategory='全部维度';
...
go=function(id){
  ...
  if(curScreen==='ce-today'&&id!=='ce-today'){ceState.entries=[];ceDraft=null;ceEditingId=null;ceTodaySearchQuery='';ceRecalcSession()}
...
ceRender=function(){
 const active=document.activeElement,selector=active?.closest('.ce-record-search')?'.ce-record-search input':active?.closest('.ce-today-search')?'.ce-today-search input':null;
...
function ceV1Scenario(kind){
 ...
 const messages={student:'学生状态错误，请重新进入',changed:'检查项已变更，请重新进入当前页面',invalid:'学生校徽无效，请重新扫码',unbound:'学生校徽尚未绑定',permission:'暂无点评权限',camera:'无摄像头权限，请打开摄像头',network:'网络错误，请重新上传'};
 toast(messages[kind]||'');
}
for(const id of ['ce-rank','ce-multi','ce-today','ce-records','ce-settings']){
 if(demoInfo[id]){
  demoInfo[id].d+=' 本文件为V1.0本地交互演示。学校端接口、扫码和上传进度使用模拟数据；正式权限与业务日期由服务端判断。';
